  ...
  return (
    ...
    <section className="luxury-page-shell chemicals-page mx-auto">
      ...
      <section className="chemicals-page__table-section">
        ...
        <h2 className="chemicals-page__table-title">{t('chemicals.tableTitle')}</h2>
          ...
          <table className="luxury-table luxury-table--cards">
            ...
            <tbody>
              ...
              {chemicals.map((chemical) => {
                ...
                const phRow = chemical.technicalInfo.find((row) => row.key === 'ph');

                const formRow = chemical.technicalInfo.find((row) => row.key === 'form');
                ...
                return (
                  ...
                  <tr key={chemical.id}>
                    ...
                    <td data-label={labels.chemical}>{localizedText(chemical.name, language)}</td>
                    ...
                    <td data-label={labels.dosage}>{localizedText(chemical.dosage, language)}</td>

                    <td data-label={labels.ph}>{phRow ? localizedText(phRow.value, language) : '—'}</td>

                    <td data-label={labels.form}>{formRow ? localizedText(formRow.value, language) : '—'}</td>
