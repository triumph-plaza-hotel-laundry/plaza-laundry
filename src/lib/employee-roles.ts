...
export function isManagerEmployee(employee: Pick<LaundryEmployee, 'id' | 'tier'>): boolean {
  return (
...
export function getShiftEligibleEmployees(employees: readonly LaundryEmployee[]): LaundryEmployee[] {
  return employees.filter((employee) => !isManagerEmployee(employee));
}

export function sortEmployeesForDisplay(employees: readonly LaundryEmployee[]): LaundryEmployee[] {
  const managerOrder = new Map(MANAGER_EMPLOYEE_IDS.map((id, index) => [id, index]));
  ...
  return [...employees].sort((left, right) => {
    ...
    if (leftIsManager && rightIsManager) {
      return (managerOrder.get(left.id as ManagerEmployeeId) ?? 99) -
