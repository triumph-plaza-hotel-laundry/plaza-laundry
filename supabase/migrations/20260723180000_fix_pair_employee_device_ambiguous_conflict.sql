-- Root cause (runtime-proven): pair_employee_device RETURNS TABLE defines
-- output column onesignal_player_id, which makes
--   ON CONFLICT (onesignal_player_id)
-- ambiguous (SQLSTATE 42702). RPC aborts; no employee_linked_devices row;
-- session stays pending; UI shows "No active device linked".
--
-- Fix: return JSONB (no RETURNS TABLE variables) and conflict on constraints.

DROP FUNCTION IF EXISTS pair_employee_device(TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN);
