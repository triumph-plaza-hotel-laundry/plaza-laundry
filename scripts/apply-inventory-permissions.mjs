...
async function run() {
  ...
  const sqlBody = readFileSync(
    join(process.cwd(), 'supabase', 'migrations', '20260711160000_inventory_permissions_and_disabled_at.sql'),
    'utf8',
  );
  ...
  if (password) {
    ...
    if (sql) {
      try {
        ...
        console.log('inventory permissions migration applied successfully (postgres).');
        return;
      } finally {
  ...
  if (accessToken) {
    ...
    console.log('inventory permissions migration applied successfully (management API).');
    return;
  }

  throw new Error(
    'No database credentials available. Add SUPABASE_DB_PASSWORD or SUPABASE_ACCESS_TOKEN to .env.local, or run supabase/manual/inventory_permissions_setup.sql in the Supabase SQL Editor.',
  );
}
