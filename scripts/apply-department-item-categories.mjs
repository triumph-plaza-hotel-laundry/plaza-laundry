...
async function run() {
  ...
  if (password) {
    ...
    if (sql) {
      try {
        ...
        console.log('department_item_categories migration applied successfully (postgres).');
        return;
      } finally {
  ...
  if (accessToken) {
    ...
    console.log('department_item_categories migration applied successfully (management API).');
    return;
  }

  throw new Error(
    'No database credentials available. Add SUPABASE_DB_PASSWORD or SUPABASE_ACCESS_TOKEN to .env.local, or run supabase/manual/department_item_categories_setup.sql in the Supabase SQL Editor.',
  );
}
